...
app.listen(PORT, HOST, async () => {
    ...
    try {
        ...
        if (donationResult) {
            ...
            logger.server('Gerenciador de canal de doação iniciado com sucesso');
        } else {
        ...
        logger.error('Erro ao iniciar gerenciador de canal de doação', { error: error.message });
    }
    ...
    try {
        ...
        if (migrationStatus.needsMigration) {
            ...
            if (migrationResult) {
                ...
                logger.server('Migração automática do config.json concluída');
            } else {
                console.log('❌ Falha na migração do config.json');
                logger.error('Falha na migração automática do config.json');
            }
        } else {
        ...
        logger.error('Erro ao executar migração automática do config.json', { error: error.message });
    }
